...
void main()
{
    uint8_t  server_ip[4]={192,168,1,1}; 
    uint16_t server_port = 12345;
    W5500_Init();
    
    HAL_Delay(2000);
    
    // Socket Send KeepAlive Every 2*5 Seconds
    W5500_SPI_WirteByte(Sn_KPALVTR(0),0x02);
    ...
    while(1)
    {
        switch( W5500_SPI_ReadByte(Sn_SR(0)) )
        {
            // Socket处于初始化完成(打开)状态
            case SOCK_INIT:
                W5500_Connect(0, server_ip,server_port);
                break;
            ...
            case SOCK_ESTABLISHED:
                ...
                if( len > 0)
                {
                    ...
                    W5500_Recv(0,buffer,len);
                    W5500_Send(0,buffer,len);
                }
                break;
            ...
            // Socket处于关闭状态
            case SOCK_CLOSED:
                // Open a Socket with No Delay Mode
                W5500_Open(0,0,Sn_MR_TCP,Sn_MR_ND);
                break;
            default : break;
        }
...
#define W5500_CS(state)     HAL_GPIO_WritePin(GPIOA,GPIO_PIN_4,state?GPIO_PIN_SET:GPIO_PIN_RESET)
...
#define W5500_RST(state)    HAL_GPIO_WritePin(GPIOC,GPIO_PIN_4,state?GPIO_PIN_SET:GPIO_PIN_RESET)
...
void    W5500_Write_TxBuffer(uint8_t s, uint8_t *data, uint16_t len);
...
/**
 @brief Socket n Tx Buffer
 */
#define Sn_TxBuffer(ch)                  (0x000010 + (ch<<5))

/**
 @brief Socket n Rx Buffer
 */
#define Sn_RxBuffer(ch)                  (0x000018 + (ch<<5))
